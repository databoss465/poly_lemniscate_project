...
def recursive_refine (p, min_cell_size:float, depth:int, max_depth:int, xlim=(-2, 2), ylim=(-2, 2), in_pts=None):
    if in_pts is None:
        in_pts = {}
    ...
    # in_count with memoization
    # in_count = sum(1 for pt in test_pts if np.abs(p(pt)) < 1.0)     
    in_count = 0
    for pt in test_pts:
        if pt in in_pts.keys():
            in_count += in_pts[pt]
        else:
            in_pts[pt] = np.abs(p(pt)) < 1.0
            in_count += in_pts[pt]

    area = (xlim[1] - xlim[0]) * (ylim[1] - ylim[0])
    cell_size = max(xlim[1] - xlim[0], ylim[1] - ylim[0])
    ...
    area1 = recursive_refine(p, min_cell_size, depth + 1, max_depth, (xlim[0], x_mid), (ylim[0], y_mid), in_pts)
    area2 = recursive_refine(p, min_cell_size, depth + 1, max_depth, (xlim[0], x_mid), (y_mid, ylim[1]), in_pts)
    area3 = recursive_refine(p, min_cell_size, depth + 1, max_depth, (x_mid, xlim[1]), (ylim[0], y_mid), in_pts)
    area4 = recursive_refine(p, min_cell_size, depth + 1, max_depth, (x_mid, xlim[1]), (y_mid, ylim[1]), in_pts)

    return area1 + area2 + area3 + area4
    
def hybrid_amr_py (roots: list[complex], xlim=(-2, 2), ylim=(-2, 2), init_divs=10, min_cell_size=1e-5, max_depth=11):
    """
...
def hybrid_amr_cpp (roots: list[complex], xlim=(-2, 2), ylim=(-2, 2), init_divs=8,
    ...
    and midpoint until the cell size is less than `min_cell_size` or the maximum depth is reached.
    """ 
    degree = len(roots)
